...
type (
	MiddlewareHandlerService interface {
		...
		PlayerIdValidation(next echo.HandlerFunc) echo.HandlerFunc
	}
...
func (h *middlewareHandler) PlayerIdValidation(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		newCtx, err := h.middlewareUsecase.PlayerIdValidation(c)
		if err != nil {
			return response.ErrResponse(c, http.StatusUnauthorized, err.Error())
		}
