...
func NewAccessToken(secret string, expAt int64, claims *Claims) AuthFactory {
	return &accessToken{
		authConcrete: &authConcrete{
			...
			Claims: &AuthMapClaims{
				...
				RegisteredClaims: jwt.RegisteredClaims{
					...
					Audience:  []string{"mmorpg-shop"},
					ExpiresAt: jwtTimeDurationCal(expAt),
...
func NewRefreshToken(secret string, expAt int64, claims *Claims) AuthFactory {
	return &refreshToken{
		authConcrete: &authConcrete{
			Secret: []byte(secret),
			Claims: &AuthMapClaims{
...
func ReloadToken(secret string, expAt int64, claims *Claims) string {
	obj := &refreshToken{
		authConcrete: &authConcrete{
			Secret: []byte(secret),
			Claims: &AuthMapClaims{
				...
				RegisteredClaims: jwt.RegisteredClaims{
					...
					Subject:   "refresh-token",
					Audience:  []string{"mmorpg-shop"},
					ExpiresAt: jwtTimeRepeatAdapter(expAt),
					NotBefore: jwt.NewNumericDate(now()),
					IssuedAt:  jwt.NewNumericDate(now()),
				},
			},
...
func ParseToken(secret string, tokenString string) (*AuthMapClaims, error) {
	...
	return nil, errors.New("error: token claims is invalid")
}
